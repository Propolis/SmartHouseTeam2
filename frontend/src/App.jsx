...
const rgbToHex = (rgb) => {
    // Разделяем строку на компоненты R, G, B
...
const App = () => {
    ...
    const fetchData = () => {
        fetch('/data') // GET-запрос для получения данных
            ...
            .then(data => {
                ...
                // Конвертация RGB в HEX и обновление состояния
                if (data.toggleRGB) {
                    const hexColor = rgbToHex(data.toggleRGB);
                    setLampColor(hexColor);
                }
    ...
    useEffect(() => {
        ...
        const interval = setInterval(fetchData, 1000); // Обновление каждую секунду
